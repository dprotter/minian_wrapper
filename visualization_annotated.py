...
class CNMFViewer:
    ...
    def __init__(
        ...
        """
        #load variables in
        #one problem here is if A is passed in, and so is minian, A and minian data labels wont match
        ...
        try:
            self.unit_labels = minian["unit_labels"].compute() #minian defaults to None, so vvvv
        except:
            #if not taking a dataset, create data labels
        ...
        #calculate the centroids of all the different cell footprints
        self.cents = centroid(self._A, verbose=True)
        print("computing sum projection")
        with ProgressBar():
            self.Asum = self._A.sum("unit_id").compute()
        
        #meta options --> but what are these?
        self._NNsort = sortNN #nearest neighbor sort
        self._normalize = False
        self._useAC = True #whether to plot AC (footprint shaded by C) instead of a static image
        self._showC = True #whether to plot C (calcium)
        self._showS = True #whether to plot S (spikes)
        
        #isolate in meta_dims any dims that arent "frame", "width", or "height"
        #dims are probably a dict, and set(dict) returns a set of the keys of the dict
        ...
        #not sure why we do this, but to take just the first value if a dim has len > 1???
        self.metas = {d: v[0] for d, v in self.meta_dicts.items()}
        
        if self._NNsort:
            try:
                #order centroids by nearest neighbor algorithm
                self.cents["NNord"] = self.cents.groupby(
                    meta_dims, group_keys=False
                ).apply(NNsort)
            except ValueError:
                #i wonder what error we are correcting for here
    ...
    def update_subs(self):
        #I think these are the subests to use
        #not sure what these metas are. subseting frame, width, height... and? unit_ids?
    ...
    def callback_f(self, f, y):
        ...
        if len(self._AC) > 0 and len(self._mov) > 0:
            ...
            #send data on holoviews pipe? maybe an entry to fastplotlib...... 
            self.pipAC.send(AC)
